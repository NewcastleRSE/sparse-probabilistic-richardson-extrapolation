...
def GRE_stepwise(X, Y, k_name):
    ...
    A = np.array([[0, 0]])  # mean function: just the intercept
    B = np.zeros((1, d))    # initial rate function: only intercept
    order = 0
    ...
    while carry_on:
        ...
        if np.any(to_include):
            B_updated = np.vstack([B, B_extra[to_include, :]])
